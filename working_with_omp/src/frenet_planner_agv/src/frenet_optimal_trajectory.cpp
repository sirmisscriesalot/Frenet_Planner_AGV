...
cerr<<"size of fplist = "<<frenet_paths.size()<<endl;
	return frenet_paths;
}
...
omp_set_num_threads(OMP_THREADS);
#pragma omp parallel for collapse(1)
    for (int Ti = int(MINT / DT); Ti <= int((MAXT + DT) / DT); Ti += 1)
    {
	double ti = double(Ti) * DT;
#pragma omp critical
...
#pragma omp parallel for collapse(2)
     for (int Ti = int(MINT / DT); Ti <= int((MAXT + DT) / DT); Ti += 1)
		{
			for (int tvv =0; tvv < int(2* N_S_SAMPLE);tvv+= 1)
			{
	double ti = double(Ti) * DT;
	double tv = TARGET_SPEED - D_T_S*N_S_SAMPLE + tvv*D_T_S;
#pragma omp critical
